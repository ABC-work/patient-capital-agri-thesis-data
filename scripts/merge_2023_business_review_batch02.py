#!/usr/bin/env python3
"""Merge verified 2023 batch 02 into v2 and write a new v3 worktable."""
...
SOURCE = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v2.csv"
BATCH = ROOT / "outputs" / "2023年报主营业务人工复核_第二批8家_v1.csv"
OUTPUT = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v3.csv"
...
def main() -> None:
    ...
    for idx, row in table.iterrows():
        ...
        table.at[idx, "口径提示"] = (
            f"严格口径占比={item['严格口径涉农收入占比']}；扩展口径占比={item['扩展口径涉农收入占比']}；"
            "两套分子详见2023年报主营业务人工复核_第二批8家_v1.csv"
        )
        updated += 1
    assert updated == 8
    assert len(table) == 958 and not table.duplicated(["股票代码", "年份"]).any()
    assert table["复核状态"].str.startswith("已人工复核", na=False).sum() == 18
    table.to_csv(OUTPUT, index=False, encoding="utf-8-sig")
    print(f"Wrote {len(table)} rows to {OUTPUT}; cumulative verified rows: 18; v1/v2 untouched.")
