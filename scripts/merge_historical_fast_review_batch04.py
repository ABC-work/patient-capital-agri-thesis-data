#!/usr/bin/env python3
"""Merge historical fast-review batch 04 while preserving all prior reviews."""
...
BATCH = ROOT / "outputs" / "跨年快速复核第四批_荃银高科9条_v1.csv"
OUTPUT = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v14_跨年快速复核第四批.csv"
...
def main():
    ...
    for i, row in merged.iterrows():
        ...
        merged.at[i, "复核状态"] = "已人工复核（跨年快速核对第四批）"
        merged.at[i, "口径提示"] = "逐年核对官方年报分产品收入；仅用明确种子项目的保守分子，不复制相邻年度。"
        n += 1
    assert n == 9 and len(merged) == 958
    assert not merged.duplicated(["股票代码", "年份"]).any()
    assert merged["复核状态"].str.startswith("已人工复核", na=False).sum() == 223
    ...
    print("v14 rows=958; new reviews=9; cumulative reviewed=223; 2023 unchanged")
