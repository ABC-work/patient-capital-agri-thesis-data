#!/usr/bin/env python3
"""Merge audited batch-16 O/P groups into v26."""
...
BATCHES = {
    "O": ROOT / "outputs" / "跨年快速复核第十六批_O组_3家公司.csv",
    "P": ROOT / "outputs" / "跨年快速复核第十六批_P组_3家公司.csv",
}
OUTPUT = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v27_跨年快速复核第十六批第一组.csv"
...
def normalize(group: str, row: pd.Series) -> dict[str, str]:
    ...
    extras = []
    for column in ["ST退市处理说明", "2022行业字段处理", "待核实点"]:
    ...
    return {
        ...
        "复核状态": f"已人工复核（跨年快速核对第十六批{group}组）",
        "口径提示": (
            "逐年核对官方年报；养殖、投入品和初加工按同一维度下界计量，贸易、屠宰、"
            "熟食和普通食品深加工排除；重列比较数优先，混合品类保留上下界。 " + " ".join(extras)
        ).strip(),
...
def main() -> None:
    ...
    assert int(verified_before.sum()) == 663
    ...
    assert int(reviewed_after.sum()) == 700
    ...
    assert int((merged["扩展样本结论"] == "是").sum()) == 359
    assert int((merged["边界样本结论"] == "是").sum()) == 43

    merged.to_csv(OUTPUT, index=False, encoding="utf-8-sig")
    print("v27 rows=958; OP=37; reviewed=700; strict=192; expanded=359; boundary=43")
