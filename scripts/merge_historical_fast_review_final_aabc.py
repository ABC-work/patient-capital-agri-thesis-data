#!/usr/bin/env python3
"""Merge audited final AA/AB/AC groups into v33 complete-review master."""
...
BATCHES = {
    "AA": ROOT / "outputs" / "跨年快速复核最终批_AA组_11家公司.csv",
    "AB": ROOT / "outputs" / "跨年快速复核最终批_AB组_11家公司.csv",
    "AC": ROOT / "outputs" / "跨年快速复核最终批_AC组_13家公司.csv",
}
OUTPUT = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v33_全量人工复核完成.csv"
...
def normalize(group: str, row: pd.Series) -> dict[str, str]:
    ...
    return {
        ...
        "复核状态": f"已人工复核（最终批{group}组）",
        "口径提示": (
            "逐年核对官方年报；主营转型按公司—年份处理；直接农业、初加工和农业投入品分别按冻结口径识别；"
            "混合业务无法可靠拆分时只保留上下界，正式分子留空。 " + " ".join(notes)
        ).strip(),
...
def main() -> None:
    ...
    assert int(reviewed_after.sum()) == 958
    ...
    assert int((merged["严格样本结论"] == "是").sum()) == 218
    assert int((merged["扩展样本结论"] == "是").sum()) == 414
    ...
    print(f"v33 rows=958; final=49; reviewed=958; pending={len(pending)}; yes={counts}")
