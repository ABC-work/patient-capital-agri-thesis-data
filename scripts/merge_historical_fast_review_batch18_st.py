#!/usr/bin/env python3
"""Merge audited batch-18 S/T groups into v29."""
...
SOURCE = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v29_跨年快速复核第十七批第二组.csv"
BATCHES = {
    "S": ROOT / "outputs" / "跨年快速复核第十八批_S组_3家公司.csv",
    "T": ROOT / "outputs" / "跨年快速复核第十八批_T组_3家公司.csv",
}
OUTPUT = ROOT / "outputs" / "涉农候选公司年份_年报主营业务复核工作表_v30_跨年快速复核第十八批.csv"
...
def normalize(group: str, row: pd.Series) -> dict[str, str]:
    ...
    return {
        ...
        "复核状态": f"已人工复核（跨年快速核对第十八批{group}组）",
        "口径提示": (
            "逐年核对官方年报；普通饮料和休闲食品深加工排除，粮食初加工按上下界；"
            "养殖与屠宰鲜品无法拆分时待核实，水产食品排除项不进入扩展分子。 "
            + " ".join(notes)
        ).strip(),
...
def main() -> None:
    ...
    assert int(verified_before.sum()) == 757
    ...
    assert int(reviewed_after.sum()) == 787
    ...
    print("v30 rows=958; ST=30; reviewed=787; strict=192; expanded=367; boundary=50")
